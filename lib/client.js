  ...
  var Client = function(kafka_options) {
    ...
    var result = {
      _get_client: function(connection_string, client_id, zk_options) {
        var client = new kafka.Client(connection_string || '127.0.0.1:2181/', client_id || 'kafka-node-consumer', zk_options);
        return client;
      },
      _start_consumer: function(client, payloads, options, on_message) {
        ...
        consumer.on('rebalancing', function () {
          console.log('KAFKA CONSUMER REBALANCING ...')
        });
        consumer.on('rebalanced', function () {
          console.log('KAFKA CONSUMER REBALANCED')
        });
        consumer.on('registered', function () {
          console.log('KAFKA CONSUMER REGISTERED')
        });
        consumer.on('brokersChanged', function () {
          console.log('KAFKA CONSUMER brokersChanged')
        });
        consumer.on('offsetOutOfRange', function () {
          console.log('KAFKA CONSUMER offsetOutOfRange')
        });
      ...
      route: function(payloads, options, routes) {
        ...
        var delay = kafka_options.zkMaxSessionTimeout || 30000;
        console.log('KAFKA CONSUMER: DELAYING START FOR ' + delay + ' MSECONDS');
        setTimeout(function() {
